...
fn trim_compressed_detailed(
    ...
    println!(
        "2. Probe format:        {:>10.3} ms",
        t_probe.elapsed().as_secs_f64() * 1000.0
    );
    ...
    println!(
        "3. Get track info:      {:>10.3} ms",
        t_track.elapsed().as_secs_f64() * 1000.0
    );
    ...
    println!(
        "4. Create decoder:      {:>10.3} ms",
        t_decoder.elapsed().as_secs_f64() * 1000.0
    );
    ...
    println!(
        "5. Seek to start:       {:>10.3} ms (target: {}, actual: {})",
    ...
    println!(
        "6. Determine channels:  {:>10.3} ms (channels: {})",
        t_channels.elapsed().as_secs_f64() * 1000.0,
        channels
    );
    ...
    println!(
        "8. Read packets:        {:>10.3} ms ({} packets)",
        total_read_time * 1000.0,
        packet_count
    );
    println!(
        "9. Decode packets:      {:>10.3} ms",
        total_decode_time * 1000.0
    );
    println!(
        "10. Convert samples:    {:>10.3} ms",
        total_convert_time * 1000.0
    );
    println!(
        "11. Write samples:      {:>10.3} ms ({} frames)",
        total_write_time * 1000.0,
        total_frames
    );
    ...
    let processing_time =
        total_read_time + total_decode_time + total_convert_time + total_write_time;
    ...
    println!(
        "  Decode:         {:.1}%",
        (total_decode_time / total_time) * 100.0
    );
    println!(
        "  Convert:        {:.1}%",
        (total_convert_time / total_time) * 100.0
    );
    println!(
        "  Write:          {:.1}%",
    ...
    println!(
        "  Packets/sec:    {:.2} K",
        (packet_count as f64 / total_time) / 1000.0
    );
    println!(
        "  Frames/sec:     {:.2} M",
        (total_frames as f64 / total_time) / 1_000_000.0
    );
    println!(
        "  Avg packet:     {:.2} frames",
        total_frames as f64 / packet_count as f64
    );
