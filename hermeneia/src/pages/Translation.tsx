...
import { MARIAN_LANGUAGES, getLanguageName } from "../types/translation";
...
const Translation: Component = () => {
  ...
  // Filter target languages - must be different from source and have Marian model
  const availableTargetLanguages = createMemo(() => {
    return MARIAN_LANGUAGES.filter(lang => lang.code !== sourceLang());
  });

  // Filter source languages - must be different from target
  const availableSourceLanguages = createMemo(() => {
    return MARIAN_LANGUAGES.filter(lang => lang.code !== targetLang());
  });
  ...
  createEffect(() => {
    if (sourceLang() === targetLang()) {
      // Pick first available target that's different
      const available = MARIAN_LANGUAGES.filter(l => l.code !== sourceLang());
  ...
  const handleTranslate = async () => {
    if (!filePath()) return;
    if (!pairSupported()) {
      setError(`No Marian model available for ${getLanguageName(sourceLang())} to ${getLanguageName(targetLang())}. Marian models only support translations to/from English.`);
      return;
    }
            ...
            <Show when={!isTranslating() && !result()}>
              <section class="settings-panel">
                ...
                <div class="model-info">
                  ...
                  <Show when={!pairSupported()}>
                    <span class="model-warning">
                      (Pair not supported - try English as source or target)
                    </span>
                  </Show>
                </div>
                ...
                <button
                  ...
                  title={!pairSupported() ? 'Language pair not supported by Marian models' : ''}
                >
                  <svg viewBox="0 0 24 24" width="20" height="20">
